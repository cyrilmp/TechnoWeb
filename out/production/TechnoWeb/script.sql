...
insert into Element(title,description,status,creation_date,updating_date,idList) values ('Pain','En bon français opn achète du pain.',0, now() ,now(), (select id from List order by id asc limit 1));
insert into Element(title,description,status,creation_date,updating_date,idList) values ('Oeuf','Pour faire un gateau',0, now() ,now(), (select id from List order by id asc limit 1));
insert into Element(title,description,status,creation_date,updating_date,idList) values ('Farine','Pour faire un gateau',0, now() ,now(), (select id from List order by id asc limit 1));

insert into Element(title,description,status,creation_date,updating_date,idList) values ('ActionMan','Le plus grand de tout les héros.',0, now() ,now(), (select id from List order by id desc limit 1));
insert into Element(title,description,status,creation_date,updating_date,idList) values ('PS4','J''arrête de bosser, j''ai une PS4',0, now() ,now(), (select id from List order by id desc limit 1));
insert into Element(title,description,status,creation_date,updating_date,idList) values ('Voiture télécommandé','Pour faire la course avec le robot aspirateur.',0, now() ,now(), (select id from List order by id desc limit 1));
